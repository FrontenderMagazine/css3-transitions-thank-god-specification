...
`delay` может принимать отрицательные значения, при которых переход будет 
...
для всех компонентов полной записи свойства (например, `padding-top`), даже если 
...
нескольких сот миллисекунд). Вам поможет `requestAnimationFrame()`. Браузер 
...
также значения относящиеся к ширине (обычно `<percentage>`), 
...
WebKit и Opera переходы выглядят логично, Firefox немного искажает результат. 
...
Оказывается, такое использвование свойства для старых IE 
...
поскольку они продолжают переводить фоновые вкладки в режим сна.
...
У браузера Opera другое мнение по этому вопросу: у них переходы 
...
До сих пор я описывал странности, обнаруженные при тестировании даннных 
...
взгляните на эти отличные сайты:
